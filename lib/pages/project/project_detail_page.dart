  ...
  Future<void> _applyProject() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('지원하기')),
        ],
      ),
    );
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('지원이 완료되었습니다!')));
      }
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('지원 실패: $e')));
    }
  }
    ...
    return Scaffold(
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                ...
                Column(
                  children: [
                    ...
                    Text('$_likeCount', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.grey[600], fontSize: 12)),
                  ],
                ),
              ],
            ),
            ...
            Container(
              ...
              child: Column(
                children: [
                  _DetailRow(icon: Icons.check_circle_outline, label: '상태', value: widget.project.isRecruiting ? '모집중' : '마감됨', isHighlight: widget.project.isRecruiting),
                  const Divider(height: 24),
                  _DetailRow(icon: Icons.people_outline, label: '모집 인원', value: '${widget.project.maxMembers}명'),
                  const Divider(height: 24),
                  Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      ...
                      const SizedBox(width: 70, child: Text('기술 스택', style: TextStyle(fontWeight: FontWeight.w600, color: Colors.grey))),
                      Expanded(
                        child: Wrap(
                          spacing: 6,
                          runSpacing: 6,
                          children: widget.project.techStack.isNotEmpty
                              ? widget.project.techStack.split(',').map((tech) => Chip(
                            label: Text(tech.trim(), style: const TextStyle(fontSize: 11, height: 1.0)),
                            padding: const EdgeInsets.symmetric(horizontal: 0, vertical: -4),
                            materialTapTargetSize: MaterialTapTargetSize.shrinkWrap,
                            backgroundColor: theme.colorScheme.primary.withValues(alpha: 0.05),
                            side: BorderSide.none,
                          )).toList()
                              : [const Text('미정', style: TextStyle(color: Colors.black87))],
                        ),
                      ),
                    ],
                  ),
            ...
            // 3. 프로젝트 설명
            Text('프로젝트 소개', style: theme.textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
                ...
                child: OutlinedButton.icon(
                  ...
                  onPressed: () {
                    Navigator.push(context, MaterialPageRoute(
                        builder: (_) => ProjectBoardPage(projectId: widget.project.id, projectTitle: widget.project.title)
                    ));
                  },
                ),
              ),
            ...
            Row(
              children: [
                Text('댓글', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                const SizedBox(width: 8),
                Text('${_comments.length}', style: TextStyle(color: theme.primaryColor, fontWeight: FontWeight.bold)),
              ],
            ),
            const SizedBox(height: 16),
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                ...
                                Text(DateFormat('MM/dd HH:mm').format(comment.createdAt), style: TextStyle(color: Colors.grey[400], fontSize: 11)),
                              ],
                            ),
      ...
      bottomNavigationBar: Container(
        ...
        child: ElevatedButton(
          onPressed: isOwner
              ? () {
            Navigator.push(context, MaterialPageRoute(builder: (context) => ApplicantListPage(projectId: widget.project.id)));
          }
              : (_hasApplied || _isCheckingApplied || !widget.project.isRecruiting)
              ? null
              : _applyProject,
          style: ElevatedButton.styleFrom(
            backgroundColor: isOwner ? const Color(0xFF10B981) : theme.primaryColor,
            padding: const EdgeInsets.symmetric(vertical: 16),
            elevation: 0,
          ),
          child: _isCheckingApplied
              ...
              : Text(
            isOwner ? '지원자 관리 (${widget.project.title})' : _hasApplied ? '지원 완료 (대기중)' : (widget.project.isRecruiting ? '프로젝트 지원하기' : '모집 마감'),
